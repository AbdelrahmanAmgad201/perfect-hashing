...
public class SquareHasherTest {

    private SquareHasher hasher;

    @BeforeEach
    void setUp() {
        hasher = new SquareHasher();
    }
    ...
    void testLargeInsertionPerformance() {
        ...
        System.out.println("Number of rehashes: " + hasher.getRehashingCount());
    ...
    void testResizing() {
        int numItems = 500; // Should trigger at least one resize
        List<String> keys = new ArrayList<>();
        ...
        int rehashCountBefore = hasher.getRehashingCount();

        for (String key : keys) {
            hasher.insert(key);
        }

        int rehashCountAfter = hasher.getRehashingCount();
        System.out.println("Rehashes performed during insertion: " + (rehashCountAfter - rehashCountBefore));
    ...
    void testOperationTiming() {
        ...
        for (int size : testSizes) {
            ...
            System.out.println("  Rehash count: " + newHasher.getRehashingCount());
        }
    }
    ...
    void testCollisionHandling() {
        // Create keys that might cause collisions
        // For this test to be effective, we'd need to know the hash function details
        // This is a simplified attempt to create potential collisions
        List<String> keys = new ArrayList<>();
        for (int i = 0; i < 1000; i++) {
            keys.add("collision_key_" + i);
        }
        ...
        int rehashCountAfter = hasher.getRehashingCount();
        System.out.println("Collisions handled (rehashes): " + (rehashCountAfter - rehashCountBefore));
